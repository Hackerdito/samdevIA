...
export const MediaModal: React.FC<MediaModalProps> = ({
  ...
  const engineObj = AI_ENGINES.find((e) => e.id === item.engine);
  ...
  const handleDownload = () => {
    ...
    a.download = `magnific-${item.engine}-${item.id}.${isVideo ? 'mp4' : 'jpg'}`;
    a.target = '_blank';
      ...
      >
        ...
        <div className="flex w-full flex-col justify-between border-t lg:border-t-0 lg:border-l border-zinc-800 bg-zinc-900/60 p-6 lg:w-96 overflow-y-auto max-h-[85vh]">
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-zinc-400 font-medium">
                    {engineObj?.provider || 'Freepik AI Engine'}
                  </span>
                </div>
              </div>
            ...
            {item.negativePrompt && (
              <div>
                <span className="text-xs font-semibold text-zinc-400 block mb-1">Prompt Negativo</span>
                <div className="rounded-lg border border-zinc-800/80 bg-zinc-950/50 p-2 text-xs text-zinc-400">
                  {item.negativePrompt}
                </div>
              </div>
            )}
            ...
            <div className="space-y-2 rounded-xl border border-zinc-800/80 bg-zinc-950/50 p-3 text-xs">
              ...
              {item.seed && (
                <div className="flex justify-between text-zinc-400">
                  ...
                  <span className="font-mono text-zinc-200">{item.seed}</span>
                </div>
              )}
          ...
          <div className="mt-6 flex flex-col gap-2 pt-4 border-t border-zinc-800">
            {item.outputUrl && (
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 py-2.5 px-4 text-xs font-bold text-zinc-950 shadow-md shadow-orange-500/20 hover:opacity-95 transition-opacity"
              >
                <Download className="h-4 w-4" />
                <span>Descargar {isVideo ? 'Video MP4' : 'Imagen HD'}</span>
              </button>
            )}
